...
# 使用 PolynomialFeatures 生成一元三次多項式特徵
degree = 3                      
...
print(f"係數 (b0, b1, b2, b3) : {coeff.round(3)}")
...
# 在第一個子圖繪製散點圖和點的連線 
ax[0].plot(X, y_poly_pred, color='g')  # 繪製一元三次迴歸線
ax[0].scatter(df.x, df.y)
ax[0].set_title("一元三次迴歸模型 - 點的連線")
